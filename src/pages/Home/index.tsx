...
export default function Home () {
  ...
  const inputUserChange = (e: any) => {
    setUser(e.target.value);
  };
  const [password, setPassword] = useState('');
  const inputPasswordChange = (e: any) => {
    setPassword(e.target.value);
  };
  const [toast400LShow, set400TLShow] = useState(false);
  const [toast500LShow, set500TLShow] = useState(false);
  const handleLogin = () => {
    fetch('https://147.15.121.92:8443/usuario/login?usuario=' + user + '&senha=' + password, {
      method: 'POST',
    })
      .then(
        (response) => {
  ...
  return (
    <main>
      <Navbar expand='lg'>
        <Container>
          <div className='navbar-brand icon-logo'><i className='bi bi-motherboard' /> MyAssist</div>
          <Navbar.Toggle aria-controls='responsive-navbar-nav' />
          <Navbar.Collapse id='responsive-navbar-nav'>
            <Nav className='me-auto'>
              <NavDropdown title='Serviços' id='collapsible-nav-dropdown'>
                <NavDropdown.Item href=''>Computadores</NavDropdown.Item>
                <NavDropdown.Item href=''>Eletrodomésticos</NavDropdown.Item>
                <NavDropdown.Item href=''>Eletrônicos</NavDropdown.Item>
              </NavDropdown>
